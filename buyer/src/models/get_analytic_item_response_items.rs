/*
 * Crossly Buyer API
 *
 * The buyer-facing Crossly API — cart, orders, wishlists, offers, cashback. A DIFFERENT principal from the seller API: authenticated by a buyer OAuth token scoped `buyer:*` and resolved by `resolveBuyer`. A seller Personal Access Token will NOT authenticate these endpoints.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 * Generated by: https://openapi-generator.tech
 */




#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct GetAnalyticItemResponseItems {
    #[serde(rename = "saleId")]
    pub sale_id: String,
    #[serde(rename = "platform")]
    pub platform: String,
    #[serde(rename = "platformOrderId", skip_serializing_if = "Option::is_none")]
    pub platform_order_id: Option<String>,
    #[serde(rename = "itemTitle", skip_serializing_if = "Option::is_none")]
    pub item_title: Option<String>,
    #[serde(rename = "itemBrand", skip_serializing_if = "Option::is_none")]
    pub item_brand: Option<String>,
    #[serde(rename = "itemSku", skip_serializing_if = "Option::is_none")]
    pub item_sku: Option<String>,
    #[serde(rename = "salePrice", skip_serializing_if = "Option::is_none")]
    pub sale_price: Option<f32>,
    #[serde(rename = "platformFee", skip_serializing_if = "Option::is_none")]
    pub platform_fee: Option<f32>,
    #[serde(rename = "shippingCost", skip_serializing_if = "Option::is_none")]
    pub shipping_cost: Option<f32>,
    #[serde(rename = "costOfGoods", skip_serializing_if = "Option::is_none")]
    pub cost_of_goods: Option<f32>,
    #[serde(rename = "netProfit", skip_serializing_if = "Option::is_none")]
    pub net_profit: Option<f32>,
    #[serde(rename = "marginPct")]
    pub margin_pct: f32,
    #[serde(rename = "daysToSell", skip_serializing_if = "Option::is_none")]
    pub days_to_sell: Option<f32>,
    #[serde(rename = "detectedAt")]
    pub detected_at: String,
}

impl GetAnalyticItemResponseItems {
    pub fn new(sale_id: String, platform: String, margin_pct: f32, detected_at: String) -> GetAnalyticItemResponseItems {
        GetAnalyticItemResponseItems {
            sale_id,
            platform,
            platform_order_id: None,
            item_title: None,
            item_brand: None,
            item_sku: None,
            sale_price: None,
            platform_fee: None,
            shipping_cost: None,
            cost_of_goods: None,
            net_profit: None,
            margin_pct,
            days_to_sell: None,
            detected_at,
        }
    }
}


