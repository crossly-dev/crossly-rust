...
/// struct for typed errors of method [`create_billing_upgrade`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateBillingUpgradeError {
...
pub async fn create_billing_upgrade(configuration: &configuration::Configuration, inline_object: crate::models::InlineObject) -> Result<crate::models::CreateBillingUpgradeResponse, Error<CreateBillingUpgradeError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/billing/upgrade", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateBillingUpgradeError> = serde_json::from_str(&local_var_content).ok();
